...
def preprocess(args):
    ...
    pid2offset = {}
    text_in_passage_path = os.path.join(
        args.wiki_dir,
        "psgs_w100.tsv" ,
    )
    text_out_passage_path = os.path.join(
        args.out_data_dir,
        "text_passages" ,
    )
    table_in_passage_path = os.path.join(
        args.wiki_dir,
        "" ,
    )
    table_out_passage_path = os.path.join(
        args.out_data_dir,
        "table_passages" ,
    )
    ...
    if os.path.exists(out_passage_path):
        ...
        print('start text passage file split processing')
        multi_file_process(args, 32, text_in_passage_path, text_out_passage_path, PassagePreprocessingFn)
        
        print('start table passage file split processing')
        multi_file_process(args, 32, table_in_passage_path, table_out_passage_path, PassagePreprocessingFn)
...
def GetProcessingFn(args, query=False):
    def fn(vals, i):
        ...
        passage_collection = [(i, passage, attention_mask, token_type_ids, MoE_type)]
...
def main():
    ...
    parser.add_argument(
        "--text_question_dir",
        type=str,
        help="location of the raw text QnA question data",
    )
    parser.add_argument(
        "--table_question_dir",
        type=str,
        help="location of the raw table QnA question data",
    )
    parser.add_argument(
        "--text_wiki_dir",
        type=str,
        help="location of the text wiki corpus",
    )
    parser.add_argument(
        "--table_wiki_dir",
        type=str,
        help="location of the table wiki corpus",
    )
    parser.add_argument(
        "--answer_dir",
        type=str,
