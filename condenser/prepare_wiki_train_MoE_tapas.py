...
from tevatron.driver.transformers_MoE import TapasTokenizer, BertTokenizer
import pandas as pd
...
tokenizer_table = TapasTokenizer.from_pretrained("google/tapas-base-finetuned-wtq")
...
with open(args.output, 'w') as f:
    for idx, item in enumerate(tqdm(data)):
        ...
        if args.add_type == 0:
            ...
            positives = []
            negatives = []
            for posx in positives_tables:
                positives.append(tokenizer_table(
                table=posx, add_special_tokens=False, max_length=256, truncation=True, padding=False)['input_ids'])
            for negx in negatives_tables:
                negatives.append(tokenizer_table(
                table=negx, add_special_tokens=False, max_length=256, truncation=True, padding=False)['input_ids'])
