...
def generate_question_vectors(
    ...
    with torch.no_grad():
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            q_ids_batch = torch.stack(batch_token_tensors, dim=0).to(torch.device("cuda:2"))
            #q_seg_batch = torch.zeros_like(q_ids_batch).cuda()
            q_seg_batch = torch.zeros_like(q_ids_batch).to(torch.device("cuda:2"))
...
def main(cfg: DictConfig):
    ...
    cfg.device =  str(torch.device("cuda:2"))
    cfg.n_gpu = 1
