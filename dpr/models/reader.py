...
def create_reader_input(
    ...
    for sample in samples:
        ...
        if len(negative_ctxs) == 0:
            print('bu yao a', len(sample.negative_passages), len(sample.passages))
...
def _create_question_passages_tensors(
    ...
    #print(len(positives_selected), len(negatives_selected))
...
def _create_token_type_ids(input_ids: torch.Tensor, sep_token_id: int, first_segment_ttid: int = 0):
    ...
    token_type_ids = torch.full(input_ids.shape, fill_value=0, dtype=torch.long)
