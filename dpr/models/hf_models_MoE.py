...
class MoE_HFBertEncoder(MoE_BertModel):
    def __init__(self, config, project_dim: int = 0):
        MoE_BertModel.__init__(self, config)
        assert config.hidden_size > 0, "Encoder hidden_size can't be zero"
        self.encode_proj = Bias_hard_MoE_Linear(config.hidden_size, project_dim) if project_dim != 0 else None
        self.init_weights()
    ...
    def init_encoder(
        ...
        if pretrained:
            # add MoE layers' parameters for pretrained model
            ...
            return MoE_HFBertEncoder(cfg, project_dim=projection_dim)
