...
class HFBertEncoder(BertModel):
    ...
    def forward(self, input_ids, attention_mask, MoE_type_tensor=None):
        hidden_states = None
...
class CoCondenser(MoE_BertModel):
    def __init__(self, config):
        MoE_BertModel.__init__(self, config)
        assert config.hidden_size > 0, 'Encoder hidden_size can\'t be zero'
        self.init_weights()
    @classmethod
    def init_encoder(cls, args, dropout: float = 0.1):
        cfg = BertConfig.from_pretrained("Luyu/co-condenser-wiki")
        if dropout != 0:
            cfg.attention_probs_dropout_prob = dropout
            cfg.hidden_dropout_prob = dropout
        return cls.from_pretrained("Luyu/co-condenser-wiki", config=cfg)
    def forward(self, input_ids, attention_mask, MoE_type_tensor=None):
        hidden_states = None
        sequence_output, pooled_output = super().forward(input_ids=input_ids,
                                                         attention_mask=attention_mask,
...
class MoE_BiEncoder(nn.Module):
    """ Bi-Encoder model component. Encapsulates query/question and context/passage encoders.
    """
    def __init__(self, args):
        super(BiEncoder, self).__init__()
        self.question_model = CoCondenser.init_encoder(args)
        self.ctx_model = CoCondenser.init_encoder(args)
    def query_emb(self, input_ids, attention_mask):
        sequence_output, pooled_output, hidden_states = self.question_model(input_ids, attention_mask)
        return pooled_output
    def body_emb(self, input_ids, attention_mask, MoE_type_tensor):
        sequence_output, pooled_output, hidden_states = self.ctx_model(input_ids, attention_mask, MoE_type_tensor)
        return pooled_output
    def forward(self, query_ids, attention_mask_q, input_ids_a = None, attention_mask_a = None, input_ids_b = None, 
            attention_mask_b = None, MoE_type_tensor = None):
        if input_ids_b is None:
            q_embs = self.query_emb(query_ids, attention_mask_q)
            a_embs = self.body_emb(input_ids_a, attention_mask_a, MoE_type_tensor)
            return (q_embs, a_embs)
        q_embs = self.query_emb(query_ids, attention_mask_q)
        a_embs = self.body_emb(input_ids_a, attention_mask_a, MoE_type_tensor)
        b_embs = self.body_emb(input_ids_b, attention_mask_b, MoE_type_tensor)
        logit_matrix = torch.cat([(q_embs*a_embs).sum(-1).unsqueeze(1), (q_embs*b_embs).sum(-1).unsqueeze(1)], dim=1) #[B, 2]
        lsm = F.log_softmax(logit_matrix, dim=1)
        loss = -1.0*lsm[:,0]
        return (loss.mean(),)
...
configs = [
    ...
    MSMarcoConfig(name="MoE_cocondenser",
                model=MoE_BiEncoder,
                use_mean=False,
                tokenizer_class=BertTokenizer,
                config_class=BertConfig,
