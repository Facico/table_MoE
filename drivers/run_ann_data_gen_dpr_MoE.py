...
def load_data(args):
    ...
    with open(train_ann_path, 'r', encoding='utf8') as f:
        # file format: q_id, positive_pid, answers, positive_MoE_type
        tsvreader = csv.reader(f, delimiter="\t")
        for row in tsvreader:
            train_pos_id.append(int(row[1]))
            train_answers.append(eval(row[2]))
            train_pos_MoE_type.append(row[3]) #eval?
            # add MoE
    ...
    with open(passage_path, "r", encoding="utf-8") as tsvfile:
        ...
        # file format: doc_id, doc_text, title, MoE_type
        for row in reader:
            if row[0] != 'id':
                passage_text[pid2offset[int(row[0])]] = (row[1], row[2], row[3])
