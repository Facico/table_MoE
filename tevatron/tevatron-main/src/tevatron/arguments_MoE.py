...
from tevatron.driver.transformers_MoE import TrainingArguments
...
class ModelArguments:
    ...
    soft_prompt: bool = field(
        default=False, metadata={"help": "soft prompt"})
    ###
...
class DataArguments:
    ...
    prompt_tokens: int = field(
        default=20,
        metadata={
            "help": "length of prompt token."
        },
    )
    initialize_from_vocab: bool = field(
        default=True, metadata={"help": "initalizes from default vocab"})
    ###
    
    def __post_init__(self):
        if self.dataset_name is not None:
            if self.dataset_name == 'json_not_encode':
                self.dataset_name = 'json_not_encode'
...
class DenseTrainingArguments(TrainingArguments):
    ...
    gc_type_chunk_size: int = field(default=32)
