...
from tevatron.driver.transformers_MoE import AutoModel, PreTrainedModel, MoE_position_BertModel
    ...
    def build(
        ...
        if os.path.isdir(model_args.model_name_or_path):
            if model_args.untie_encoder:
                ...
                lm_q = MoE_position_BertModel.from_pretrained(
                    _qry_model_path,
                    **hf_kwargs
                )
                logger.info(f'loading passage model weight from {_psg_model_path}')
                lm_p = MoE_position_BertModel.from_pretrained(
                    _psg_model_path,
                    **hf_kwargs
                )
            else:
                lm_q = MoE_position_BertModel.from_pretrained(model_args.model_name_or_path, **hf_kwargs)
                lm_p = lm_q
        # load pre-trained
        else:
            lm_q = MoE_position_BertModel.from_pretrained(model_args.model_name_or_path, **hf_kwargs)
    ...
    def build(
        ...
        if os.path.isdir(model_name_or_path):
            ...
            if os.path.exists(_qry_model_path):
                ...
                lm_q = MoE_position_BertModel.from_pretrained(model_name_or_path, **hf_kwargs)
                lm_p = lm_q
        else:
            logger.info(f'try loading tied weight')
            logger.info(f'loading model weight from {model_name_or_path}')
