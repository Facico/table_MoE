    ...
    def build(
        ...
        if os.path.isdir(model_args.model_name_or_path):
            if model_args.untie_encoder:
                ...
                lm_q = AutoModel.from_pretrained(model_args.model_name_or_path, **config_text)
                lm_p_text = lm_q
                lm_p_table = AutoModel.from_pretrained(model_args.table_model_name_or_path, **config_table)
        # load pre-trained
        else:
            lm_q = AutoModel.from_pretrained(model_args.model_name_or_path, **config_text)
            lm_p_text = copy.deepcopy(lm_q) if model_args.untie_encoder else lm_q
            lm_p_table = AutoModel.from_pretrained(model_args.table_model_name_or_path, **config_table) if model_args.untie_encoder else lm_q
    ...
    def build(
        ...
        if os.path.isdir(model_name_or_path):
            ...
            if os.path.exists(_qry_model_path):
                ...
                lm_p_text = lm_q
                lm_p_table = AutoModel.from_pretrained(table_model_name_or_path, **config_table)
        else:
            logger.info(f'try loading tied weight')
            logger.info(f'loading model weight from {model_name_or_path}')
