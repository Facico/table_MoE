...
@dataclass
class MoE_position_TableQuestionAnsweringOutput(ModelOutput):
    """
...
def load_tf_weights_in_tapas(model, config, tf_checkpoint_path):
    ...
    for name, array in zip(names, arrays):
        ...
        if isinstance(model, MoE_position_TapasForSequenceClassification):
        ...
        if isinstance(model, MoE_position_TapasModel):
        ...
        if isinstance(model, MoE_position_TapasForMaskedLM):
        ...
        for m_name in name:
            ...
            # cell selection heads
                ...
                if not isinstance(model, MoE_position_TapasForMaskedLM):
...
class MoE_position_TapasEmbeddings(nn.Module):
    """
    ...
    def __init__(self, config):
        ...
        # position embeddings
        self.position_embeddings_text = nn.Embedding(config.max_position_embeddings, config.hidden_size)
        self.position_embeddings_table = nn.Embedding(config.max_position_embeddings, config.hidden_size)
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None):
        ...
        if position_ids is None:
            ...
            if self.config.reset_position_index_per_cell:
                ...
                col_index = MoE_position_IndexMap(token_type_ids[:, :, 1], self.config.type_vocab_sizes[1], batch_dims=1)
                # shape (batch_size, seq_len)
                row_index = MoE_position_IndexMap(token_type_ids[:, :, 2], self.config.type_vocab_sizes[2], batch_dims=1)
                # shape (batch_size, seq_len)
                full_index = MoE_position_ProductIndexMap(col_index, row_index)
...
class MoE_position_TapasSelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoE_position_TapasSelfOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoE_position_TapasAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = MoE_position_TapasSelfAttention(config)
        self.output = MoE_position_TapasSelfOutput(config)
...
class MoE_position_TapasIntermediate(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoE_position_TapasOutput(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class MoE_position_TapasLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = MoE_position_TapasAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = MoE_position_TapasAttention(config)
        self.intermediate = MoE_position_TapasIntermediate(config)
        self.output = MoE_position_TapasOutput(config)
...
class MoE_position_TapasEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([MoE_position_TapasLayer(config) for _ in range(config.num_hidden_layers)])
...
class MoE_position_TapasPooler(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class MoE_position_TapasPredictionHeadTransform(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.dense = nn.Linear(config.hidden_size, config.hidden_size)
        if isinstance(config.hidden_act, str):
...
class MoE_position_TapasLMPredictionHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.transform = MoE_position_TapasPredictionHeadTransform(config)
...
class MoE_position_TapasOnlyMLMHead(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.predictions = MoE_position_TapasLMPredictionHead(config)
...
class MoE_position_TapasPreTrainedModel(PreTrainedModel):
    """
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MoE_position_TapasEncoder):
...
class MoE_position_TapasModel(MoE_position_TapasPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.embeddings = MoE_position_TapasEmbeddings(config)
        self.encoder = MoE_position_TapasEncoder(config)

        self.pooler = MoE_position_TapasPooler(config) if add_pooling_layer else None
...
class MoE_position_TapasForMaskedLM(MoE_position_TapasPreTrainedModel):
    config_class = TapasConfig
    base_model_prefix = "tapas"
    ...
    def __init__(self, config):
        ...
        self.tapas = MoE_position_TapasModel(config, add_pooling_layer=False)
        self.cls = MoE_position_TapasOnlyMLMHead(config)
...
class MoE_position_TapasForQuestionAnswering(MoE_position_TapasPreTrainedModel):
    def __init__(self, config: TapasConfig):
        super().__init__(config)

        # base model
        self.tapas = MoE_position_TapasModel(config)
    ...
    @add_start_docstrings_to_model_forward(TAPAS_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    @replace_return_docstrings(output_type=MoE_position_TableQuestionAnsweringOutput, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        input_ids=None,
        attention_mask=None,
        ...
        row_index = MoE_position_IndexMap(
        ...
        col_index = MoE_position_IndexMap(
        ...
        cell_index = MoE_position_ProductIndexMap(row_index, col_index)
        ...
        return MoE_position_TableQuestionAnsweringOutput(
...
class MoE_position_TapasForSequenceClassification(MoE_position_TapasPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class MoE_position_AverageApproximationFunction(str, enum.Enum):
    RATIO = "ratio"
...
class MoE_position_IndexMap(object):
...
class MoE_position_ProductIndexMap(MoE_position_IndexMap):
...
def flatten(index, name="segmented_flatten"):
    ...
    *num_segments* * (k - 1). The result is a tensor with *num_segments* multiplied by the number of elements in the
    ...
    return MoE_position_IndexMap(indices=indices.view(-1), num_segments=index.num_segments * batch_size, batch_dims=0)
...
def range_index_map(batch_shape, num_segments, name="range_index_map"):
    ...
    return MoE_position_IndexMap(indices=indices, num_segments=num_segments, batch_dims=list(batch_shape.size())[0])
...
def _calculate_expected_result(
    ...
    if avg_approximation == MoE_position_AverageApproximationFunction.RATIO:
        average_result = sum_result / (count_result + EPSILON_ZERO_DIVISION)
    elif avg_approximation == MoE_position_AverageApproximationFunction.FIRST_ORDER:
    ...
    elif avg_approximation == MoE_position_AverageApproximationFunction.SECOND_ORDER:
        # The sum of all probabilities except that correspond to other cells
        ex = torch.sum(scaled_probability_per_cell, dim=1, keepdim=True) - scaled_probability_per_cell + 1
