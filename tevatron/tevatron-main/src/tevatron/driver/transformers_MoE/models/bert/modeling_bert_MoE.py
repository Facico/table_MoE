...
class MoE_BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if is_cross_attention and past_key_value is not None:
            ...
            value_layer = self.transpose_for_scores(self.value(encoder_hidden_states, MoE_type_tensor=MoE_type_tensor))
            attention_mask = encoder_attention_mask
...
class MoE_BertAttention(nn.Module):
    ...
    def prune_heads(self, heads):
        ...
        # Prune linear layers
        # add MoE
        self.self.query = MoE_prune_linear_layer(self.self.query, index)
        self.self.key = MoE_prune_linear_layer(self.self.key, index)
        self.self.value = MoE_prune_linear_layer(self.self.value, index)
        self.output.dense = MoE_prune_linear_layer(self.output.dense, index, dim=1)
...
class MoE_BertPreTrainedModel(PreTrainedModel):
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, MoE_BertEncoder):
